...
public class Player {
	...
	public void changeWallet(int money) {
 	...
 	public int getTotalWorth() {
 		int valofHouses = 0; 
 		int valofHotels = 0; 
 		int valofProperties = 0;
 		int totalWorth = 0;
 		for(int property=0;property<=22;property++) {
 			if(myProperties[property].getOwner()==this) {	//counts through all properties until a property the current player owns is found, then takes the full value of the property and houses and adds it to the total, then continues on.
